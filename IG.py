...
method, menudump, xxkontol, userAGENT, pelerXD = [], [], [], [], []
s = requests.Session()
...
def _UserAgent_():
    rc = random.choice
    USN = str(rc([
         "Mozilla/5.0 (Linux; U; Android 4.4.4; en-us; MI 3W Build/KTU84P) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/39.0.0.0 Mobile Safari/537.36 XiaoMi/MiuiBrowser/2.1.1",
...
\t\t     {K2} InstaBrayen {P2}|{H2} CodeBy BrayennnXD """,title=f"{R2}{waktu()}",width=100,padding=(0,6),style=f"{color_table}"))
...
def key():
	...
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token===&ProductId=17483&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}selamat lisensi yang anda masukan terdaftar ke server InstaBrayen",width=80,padding=(0,6),style=f"{color_table}"));time.sleep(4);_loginPILL_()
	except KeyError:
		prints(Panel(f"{P2}lisensi kamu sudah kedaluwarsa silahkan beli lisensi ke admin",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()

###----------[ CEK LISENSI ]---------- ###
def cek():
	try:x=open("data/lisensi.txt","r").read()
	except FileNotFoundError:key()
	try:x = requests.get("https://app.cryptolens.io/api/key/Activate?token=WyIyODcxODY3OSIsIlkwRHVlQXhTWVRFcHIwQnF1Q1kyYzd6aUIzcDQ1OGxFdk84UTBqd1ciXQ==&ProductId=17483&Key=%s"%(x)).json()['licenseKey']['key'];_loginPILL_()
	except KeyError:
		prints(Panel(f"{P2}lisensi kamu sudah kedaluwarsa silahkan beli lisensi ke admin",width=80,padding=(0,6),style=f"{color_table}"));os.system("rm -rf data/lisensi.txt");os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
def key():
	...
	elif key in ["trial","Trial","TRIAL"]:TrialSen()
	try:ses = requests.Session();asu = ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIyODcxODY3OSIsIlkwRHVlQXhTWVRFcHIwQnF1Q1kyYzd6aUIzcDQ1OGxFdk84UTBqd1ciXQ==&ProductId=17483&Key=%s&Sign=True"%(key)).json()['licenseKey']['key'];open("data/lisensi.txt","w").write(key);prints(Panel(f"{P2}selamat lisensi yang anda masukan terdaftar ke server InstaBrayen",width=80,padding=(0,6),style=f"{color_table}"));time.sleep(4);_loginPILL_()
	except KeyError:
		prints(Panel(f"{P2} lisensi yang anda masukan tidak terdaftar silahkan beli terlebih dahulu",width=80,padding=(0,1),style=f"{color_table}"));os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+crack+Instagram");time.sleep(2);exit()
...
def TrialSen():
    try:
        print(f" {H}• {P}anda akan di arahkan ke website")
        os.system("xdg-open https://good.xyaacodee.my.id/maintenance/");exit()
    except:pass
...
def beli_bang():
    ...
    elif zxc in ["1","01"]:os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+1+minggu");time.sleep(2);exit()
    elif zxc in ["2","02"]:os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+1+bulan");time.sleep(2);exit()
    elif zxc in ["3","03"]:os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+2+bulan");time.sleep(2);exit()
    elif zxc in ["4","04"]:os.system("xdg-open https://wa.me/+16143244921?text=assalamualaikum+bang+mau+beli+lisensi+permanen");time.sleep(2);exit()
